...
export class PlayerEngine {
  ...
  resumeMaskedFromWord(idx, pos) {
    if ("masked" !== this.st.mode) return;
    ...
    ((this.st.masked = {
      ...this.st.masked,
      [verse.key]: maskStateFromWord(pos, peeks),
    }),
      (this.st.curWord = pos));
  }
  restoreMaskedAfterOneshot() {
    if ("masked" !== this.st.mode) return;
    let v = this.currentVerse();
    if (!v) return;
    this.st.curWord = this.maskStateFor(v).maxRev;
  }
  revealTo(e, t) {
  ...
  finishOneshot() {
    this.st.oneshot = null;
    this.restoreMaskedAfterOneshot();
    this.pauseAudio();
