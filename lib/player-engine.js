...
export class PlayerEngine {
  ...
  stepMasked(delta) {
    let dest = destIndexInPassage(
      this.st.vIdx,
      this.st.verses.length,
      delta,
    );
    if (null == dest) {
      if (delta < 0) {
        if (
          "function" == typeof this.onNeedPrevStop &&
          this.onNeedPrevStop()
        )
          return;
        let v = this.currentVerse();
        (v && this.restartMaskOnVerse(this.st.vIdx, v),
          (this.st.curWord = 0),
          this.armSeek(0, this.st.playing),
          this.notify());
        return;
    ...
    let verse = this.st.verses[dest];
    if (!verse) return;
    ((this.st.loop = null),
      this.restartMaskOnVerse(dest, verse),
      this.loadVerseAudio(dest, this.st.playing));
  }
  setRate(e) {
  ...
  restartMaskIfNeeded(idx) {
    if (!shouldRestartMaskOnJump(this.st.mode)) return;
    let verse = this.st.verses[idx];
    verse && this.restartMaskOnVerse(idx, verse);
  }
  jumpToVerse(e, t) {
    ((this.st.loop = null),
      this.restartMaskIfNeeded(e),
      this.loadVerseAudio(e, void 0 === t || !!t));
  }
  setMode(e) {
  ...
  ensureMaskState(e) {
    this.st.masked[e.key] ||
      (this.st.masked = {
        ...this.st.masked,
        [e.key]: freshMaskForVerse(),
      });
  }
  restartMaskOnVerse(idx, verse) {
    this.doneVerses.delete(idx);
    let peeks =
      (this.st.masked[verse.key] && this.st.masked[verse.key].peeks) ?? 3;
    this.st.masked = {
      ...this.st.masked,
      [verse.key]: freshMaskForVerse(peeks),
    };
  }
  revealTo(e, t) {
  ...
  finishWordReps() {
    this.bumpWordRepsToken();
    let w = wordRepsResumeWord(this.st);
    ...
    let verse = this.currentVerse(),
      seg = verse && segForWord(this.segsForVerse(verse), w);
    if (seg) this.armSeek(seg.end, !1);
    this.notify();
  }
  /**
  ...
  tapWordRep(e, t) {
    ...
    e !== this.st.vIdx && this.loadVerseAudio(e, !1);
    let p = this.st.wordPick || emptyWordPick();
    // After the first pin, tapping another word offers Pin+X on that end.
    if (null != p.start && null == p.end && t !== p.start) {
      this.offerWordRepEnd(t);
      return;
    }
  ...
  /** Show Pin+X on the intended last word; do not complete the range yet. */
  offerWordRepEnd(t) {
    let p = this.st.wordPick || emptyWordPick();
    if (null == p.start || null != p.end || t === p.start) return;
    if (null != p.vIdx && p.vIdx !== this.st.vIdx) return;
    ((this.st.wordPick = { ...p, open: t, nudge: !1 }),
      (this.st.wordStep = {
        ...this.st.wordStep,
        w: t,
        playedTimes: 0,
  ...
  async beginRelay(e, t, s, r) {
    if (!this.requirePlus("practice")) return !1;
  ...
  /** Leave Word Reps / Masked / Relay and keep the current ayah. */
  leavePractice() {
    let e = this.st.mode;
    if ("word" === e) {
      this.finishWordReps();
