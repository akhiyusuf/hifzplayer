-- Diras user rows. Neon is the database; Cloudflare Workers run the app.
-- The Worker applies this on first query (lib/db/schema.ts). Pasting here is optional.
-- Quran audio does not belong here — use R2.
-- Identity: email + password, Google, OTP only for password reset. Turnstile is bot-check only.
